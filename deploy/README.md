...
## Keeping RBAC in sync with the chart (anti-drift)

`deploy/k8shark.yaml` is **hand-maintained**, not byte-for-byte generated by
`helm template` (it is a single file, carries `__REGISTRY__`/`__TAG__`
placeholders, and adds PSA namespace labels the chart doesn't emit). To stop
its cluster-scoped RBAC from silently drifting wider than the canonical chart,
the hub's enrichment `ClusterRole` is pinned to exactly what the chart grants
and what `internal/hub/k8s.go` actually reads:

- core `pods`, `services` — `get`, `list`
- apps `replicasets` — `get`, `list`

No `watch`, and none of `endpoints`/`namespaces`/`nodes`/`endpointslices`/
`deployments`/`daemonsets`/`statefulsets`. `internal/hub/k8s.go` only calls
`listPods` / `listServices` / `listReplicaSetOwners`, so anything beyond the
list above is unused privilege handed to whoever `kubectl apply`s this file.

`make deploy-manifest-check` enforces this: it renders the chart's ClusterRole
and compares its RBAC rules (order-insensitive) against this file's, failing if
`deploy/` grants anything the chart doesn't. CI runs the same check in the
`helm-lint` job on every PR.

If it reports drift, reconcile by hand — render the chart's ClusterRole and
copy its `rules:` into `deploy/k8shark.yaml`'s `k8shark-enrich` ClusterRole
(the resource names differ on purpose — chart `k8shark-hub-enrich`, deploy
`k8shark-enrich` — so only the `rules` are compared):

```sh
helm template helm/k8shark | \
  yq 'select(.kind == "ClusterRole").rules'   # reference rules to mirror
make deploy-manifest-check                     # re-run until it passes
```
