...
- **mcp** (`k8shark mcp`): hand-rolled JSON-RPC-over-stdio MCP server (stdlib only, no SDK) that exposes the
  hub's data to AI agents (`get_stats`, `list_entries`, `get_entry`, `get_traffic_summary`, `get_timeline`,
  `get_workers`, `list_filter_fields`, `list_namespaces`, `list_workloads`, and a gated `start_pcap`
  placeholder). Aggregations run hub-side (`/api/summary`, `/api/timeline`, `/api/workers`); the MCP mostly
  relays.
...
`elapsedMs`, `http.method`, `response.status`, `src.name`, `dst.namespace`, `dst.workload`, `request.path`,
`redis.command`, `postgres.query`, `bytes`, `packets`, `flags`). Operators: `== != contains > < >= <=`;
boolean `and`/`or`/`not`; a bare token = full-text match. **Unknown field names are a compile error** (never
a silent match-nothing) — so `fieldGetter` returns nil for unknown fields, and every catalog entry must have
a getter (see `TestFieldCatalogMatchesGetter`). Filtering runs **hub-side** (REST `?filter=` plus optional
`?since=`/`?until=`, and the front WebSocket, live via a `MsgFilter` control frame). Keep `fieldGetter` and
the front autocomplete (`internal/hub/facets.go`, `/api/fields`) in sync when adding a field.
